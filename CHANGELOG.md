...
- **The context guard fired far too early on large-context models.** It assumed a 200,000-token window, so on a 1M-token model a session that was 18% full was reported as ~89% full and interrupted with an emergency "save your knowledge base now". It now uses the real context percentage Claude Code reports — including your actual window size, picked up automatically if you switch models mid-session — and only falls back to estimating when that isn't available (unattended runs), where it still uses the real window size if it has ever seen one. Each session tracks its own context reading, so running two Claude Code sessions at once no longer lets one session's usage mask the other's.
