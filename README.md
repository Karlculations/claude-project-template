...
- **Usage guard — stop at ≥95%.** A `UserPromptSubmit` hook blocks new prompts once the 5-hour window passes the threshold (default 95%, `CLAUDE_USAGE_THRESHOLD`), telling you the reset time. A `PreToolUse` hook denies expensive tool calls mid-turn — subagents, workflows, web, MCP, **and `Bash`** — with an instruction Claude *sees*: persist state via `/end-session`, then launch autopilot in the background (unless you asked it to wait, or the task needs your input) so the work self-resumes after the reset with no manual restart. Bash is matched because a long stretch of build/test/edit work otherwise fires no check at all and sails straight through the limit; the autopilot handoff command is allow-listed so the escape hatch still runs, and file writes stay allowed so the `/end-session` dump can complete. Bypass once with `CLAUDE_AUTONOMY=off`.
- **Fan-out reserve — don't start what can't be stopped.** A workflow spawns its sub-agents through its own runtime, so no hook can see or halt them once it is running; if they run out of window mid-flight it returns partial results and still reports success — nobody is told. So *starting* one is refused earlier than ordinary work: at 80% (`CLAUDE_USAGE_FANOUT_THRESHOLD`) rather than 95%, reserving enough window to actually finish it, with the refusal pointing at doing the work inline where every step stays guarded. Between 80% and 95% nothing else is affected. And the moment a workflow returns, the guard takes a **fresh** reading — the first trustworthy one since it started — and if the window is spent it tells Claude the results may be incomplete and to hand off rather than build on them.
- **Context guard — `/end-session` before the window fills.** A `Stop` hook reads the real context percentage Claude Code reports (cached by the status line, so it knows your actual window size — 200k or 1M); past the threshold (default 80%, `CLAUDE_CONTEXT_THRESHOLD`) it blocks the stop once per session and instructs Claude to run the `/end-session` knowledge steps immediately — so the knowledge base is written *while the details still exist*, not after compaction ate them.
