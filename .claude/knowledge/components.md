...
- `statusline.sh` — SENSOR. Claude Code pipes official `rate_limits` (5h/7d % + reset times, Pro/Max CC≥2.1) to the statusline; the script renders it and caches a normalized copy (note: `resets_at` arrives as epoch seconds here but ISO-8601 elsewhere — both shapes must be handled, see MISTAKE-011). It is ALSO the context sensor — the same payload carries `context_window.used_percentage` + `.context_window_size`, cached separately to `context-state-<session_id>.json` for `context-guard.sh` — **per-session**, because context belongs to one session while the usage window is account-wide; a shared file let a concurrent session's reading mask this one's (session_id is filename-sanitized on both sides; stale files pruned on new-session write) (separate file: an API-key user has no `rate_limits` but does have a context window, and usage-guard rewrites the usage file from its API fallback). The payload additionally exposes `model.id` (full, incl. the `[1m]` suffix), `version`, `cost`, `exceeds_200k_tokens` — check it before estimating anything. Caches to `${CLAUDE_USAGE_STATE:-$XDG_RUNTIME_DIR/claude-usage-state-$(id -u).json}`.
- `usage-guard.sh` — ACTUATOR. UserPromptSubmit: exit 2 blocks prompts at ≥95% (`CLAUDE_USAGE_THRESHOLD`). PreToolUse (matcher `Task|Agent|Workflow|WebFetch|WebSearch|mcp__.*|Bash`): JSON `permissionDecision: deny` whose reason instructs Claude to run /end-session, then — ONLY with a standing user go-ahead for unattended continuation — background-launch autopilot (`nohup .claude/autopilot.sh "continue: <summary>" &`) before stopping, so the task self-resumes after reset. **Bash IS matched** (2026-07-24): a long run of build/test/edit calls was the hole — it fired no guard check at all. The handoff is itself a Bash call, so a command containing `autopilot.sh` is allow-listed (exit 0 = "no opinion"; normal permission rules still apply — a usage carve-out, not a security bypass). `Write`/`Edit`/`Read` stay unmatched on purpose: the deny message asks for knowledge-file writes. **Fan-out is handled at three checkpoints** (2026-07-24), because a `Workflow` spawns agents in its own runtime — no hook fires for them and nothing can halt it mid-run (MISTAKE-010): *before* — PreToolUse denies **starting** a Workflow at ≥`CLAUDE_USAGE_FANOUT_THRESHOLD` (80%), reserving headroom to finish it; ordinary work is untouched between 80% and 95%, only the unguardable call is refused. *during* — the normal ≥95% deny. *after* — PostToolUse (matcher `Task|Agent|Workflow`) forces a **fresh** reading (`FETCH_TTL=0`, bypassing the cache, since cached state can be a whole workflow out of date) and over threshold exits 2 so stderr reaches Claude: results are PARTIAL, hand off. Exit 2 not JSON — the tool already ran, and stderr-to-Claude is the version-stable PostToolUse channel. The blind window is irreducibly the workflow's own duration. Failed API fetches back off `CLAUDE_USAGE_FETCH_BACKOFF` (120s) — at one guard call per Bash a 6s curl timeout each time would stall the session. Also a CLI: `--status` / `--json`. Headless fallback: undocumented `GET api.anthropic.com/api/oauth/usage` (needs `claude-code/*` User-Agent or it 429s).
- `context-guard.sh` — Stop hook. Reads the **official** context percentage cached by `statusline.sh` in `context-state.json` (`CLAUDE_CONTEXT_STATE`, TTL `CLAUDE_CONTEXT_STATE_TTL` 600s); at ≥80% (`CLAUDE_CONTEXT_THRESHOLD`) blocks the stop ONCE per session (marker file keyed by session_id) demanding /end-session Steps 1–4. Respects `stop_hook_active`. Falls back to estimating from the transcript's last `message.usage` only when that cache is missing/stale (headless), and then prefers the cached real `context_window_size` over the `CLAUDE_CONTEXT_WINDOW` 200000 default. **That default was the bug** (2026-07-24, MISTAKE-012): it read 89% on a 1M-context model at 18% real usage and fired a false emergency.
